...
public class PersonOneToOneCustomRepositoryImpl implements PersonOneToOneCustomRepository {
    ...
    @Override
    public Optional<List<PersonOneToOne>> searchByName(String name) {
        return Optional.ofNullable(
                jpaQueryFactory
                        .selectFrom(personOneToOne)
                        .leftJoin(personOneToOne.phoneOneToOne, phoneOneToOne).fetchJoin()
    ...
    @Override
    public Optional<List<PersonOneToOne>> searchAll() {
        return Optional.ofNullable(
                jpaQueryFactory
                        .selectFrom(personOneToOne)
                        .leftJoin(personOneToOne.phoneOneToOne, phoneOneToOne).fetchJoin() // 휴대폰이 없는 사람이라도 조회 목록에 포함시키고 싶어서 leftJoin 사용
                        .fetch()
        );
    }
    ...
    public void updatePersonNameByPhone(String name, String phone) {
        jpaQueryFactory
                ...
                .where(personOneToOne.id.in(
                        JPAExpressions
                                .select(phoneOneToOne.person.id)
                                .from(phoneOneToOne)
                                .where(phoneOneToOne.phone.eq(phone)))
                )
                .execute();
